...
from model import EncoderSwedish, Decoder, DecoderNER
...
if __name__ == '__main__':
    ...
    # test LibriSpeech
    features_test = prepare_data.load_features_combined('../../TSD/augmented_labels/data/normalized/features/swedish/test.npy')
    target_test = prepare_data.load_transcripts('../../TSD/augmented_labels/data/normalized/transcripts/swedish/test.txt')
    tags_test = prepare_data.load_tags('../../TSD/augmented_labels/data/normalized/ner/swedish/ner_test.txt')
    ...
    encoder = EncoderSwedish(features_test[0].size(1), encoder_hidden_size, encoder_layers, len(char2idx_ctc), batch_size, device).to(device)
    ...
    # load the model
    checkpoint = torch.load('weights/swedish_big/state_dict_11.pt', map_location=torch.device('cpu'))
